...
DOMAIN_RULES = {
    "digital": {
        "include": ["digital", "software", "platform", "data", "cloud", "workflow", "automation", "BIM"],
...
if prompt:
    ...
    # ==========================================
    # Build the web search query
    # ==========================================

    query_parts = []

    if domain_keys:
        query_parts.extend(domain_keys)

    if stage_key:
        query_parts.append(stage_key)

    if location_value:
        query_parts.append(location_value)
    ...
    filtered["Source"] = "Local"

    if not web_results.empty:

        web_results = web_results.rename(columns={
            "Title": "Name",
            "Snippet": "Description",
            "URL": "Website"
        })

        web_results["Source"] = "Web"

        for col in filtered.columns:
            if col not in web_results.columns:
                web_results[col] = ""

        web_results = web_results[filtered.columns]

        combined = (
            pd.concat([filtered, web_results], ignore_index=True)
            .drop_duplicates(subset="Name", keep="first")
        )


        st.write("Local:", len(filtered))
        st.write("Web:", len(web_results))
        st.write("Combined:", len(combined))
    ...
    if combined.empty:
        ...
        reply = "No startups found locally or on the web."
        table = None
        ...
        if intent == "count":
            ...
            if domain_keys:
                reply = f"I found {len(combined)} startups matching {domain_keys}."
            elif location_value:
                reply = f"I found {len(combined)} startups based in {location_value}."
            else:
                reply = f"I found {len(combined)} relevant startups."
            ...
            if stage_col:
                stages = combined[stage_col].dropna().astype(str).value_counts()
                if stages.empty:
